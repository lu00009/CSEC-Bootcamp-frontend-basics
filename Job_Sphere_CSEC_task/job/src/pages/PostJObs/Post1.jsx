  ...
  return (
    <form onSubmit={formik.handleSubmit}>
      ...
      <div  className="mt-6 w-[320px] h-[30px] border-[1px] rounded-[6px] outline-none p-0.5 ">
        {" "}
        <input type="text" placeholder="title" className="pl-1" id="title" name="title" {...formik.getFieldProps('title')}   />
      </div>
      {formik.touched.title && formik.errors.title ? (<div className='text-red-600'>{formik.errors.title}</div>):null}

      <div  className="mt-6 w-[320px] h-[30px] border-[1px] rounded-[6px] outline-none p-0.5">
        {" "}
        <input type="text" placeholder="type" className="pl-1" id="type" name="type" {...formik.getFieldProps('type')}  />
      </div>
      {formik.touched.type && formik.errors.type ? (<div className='text-red-600'>{formik.errors.type}</div>):null}

      <div  className="mt-6 w-[320px] h-[30px] border-[1px] rounded-[6px] outline-none p-0.5">
        {" "}
        <input type="text" placeholder="salary" className="pl-1" id="salary" name="salary" {...formik.getFieldProps('salary')} />
      </div>
      {formik.touched.salary && formik.errors.salary ? (<div className='text-red-600'>{formik.errors.salary}</div>):null}

      <div  className="mt-6 w-[320px] h-[30px] border-[1px] rounded-[6px] outline-none p-0.5">
        {" "}
        <input type="text" placeholder="description" className="pl-1" id="description" name="description" {...formik.getFieldProps('description')} />
      </div>
